...
export default function ConfigPage() {
  ...
  // Atualiza valor localmente, validando negativos e não inteiros
  function handleChange(idx, value) {
    ...
    // só aceita dígitos (inteiros não-negativos)
    if (!/^\d+$/.test(value)) {
      alert("Somente inteiros não-negativos são permitidos.");
      return;
    }
    setConfigs(cfgs =>
      cfgs.map((c, i) => (i === idx ? { ...c, value } : c))
  ...
  // Salva todas configs, ignorando vazios
  async function handleSave() {
    ...
    try {
      ...
      window.dispatchEvent(new Event("api_update_time_updated"));
  ...
  return (
    ...
    >
      {/* HEADER: título à esquerda, botões Home e Salvar à direita */}
      <div
        style={{
          width: "100%",
          background: "#fff",
      ...
      >
        ...
        {/* CONTROLES (Home + Salvar) */}
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginRight: 50 }}>
          <button
            style={{
              border: "none",
              background: "none",
          ...
          <button
            ...
            disabled={saving}
          >
            <FaSave size={21} style={{ marginRight: 7 }} />
            Salvar
          </button>
        </div>
      </div>
